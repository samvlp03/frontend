...
const Diagnose = () => {
  ...
  const [useChatOnly, setUseChatOnly] = useState(false); // 👈 toggle for text-only mode
  const { user } = useAuth();
  ...
  const handleStartVideo = async () => {
    try {
      ...
      if (videoRef.current) {
        ...
        // Wait 500ms to stabilize the feed
        setTimeout(() => {
  ...
  const handleAnalyzeVisual = async () => {
    try {
      ...
      if (!base64Image.includes("base64,")) {
        console.error("Invalid base64 data:", base64Image.slice(0, 100));
        toast.error("Image format invalid. Please try again.");
        return;
      }
  
      const res = await apiServices.analyzeVisual({
        image: base64Image,
      });
  
      setStatus({
        current_emotion: res.visual_emotion,
  ...
  const handleDiagnosis = async () => {
    ...
    try {
      ...
      const { audio_analysis, text_analysis, visual_analysis = {} } = analysisResponse.data;

      const audioConf = audio_analysis?.confidence || 0;
      const textConf = text_analysis?.confidence || 0;
      const visualConf = visual_analysis?.confidence || 0;

      const confSources = [audioConf, textConf, visualConf].filter(c => c > 0);
      ...
      const payload = {
        user_id: user.id,
        diagnosis: text_analysis?.prediction || "Depression",
